...
//        	e.printStackTrace();
	...
	public Cliente adicionarCliente(int numDoc, String tipoDocumento, String nombre, String correo, int idHotel, int idHabitacion) 
	{
		...
		{
			tx.begin();
			long tuplasInsertadas = sqlCliente.adicionarCliente(pmf.getPersistenceManager(), numDoc, tipoDocumento, nombre, correo);
			tx.commit();

			log.trace ("Inserción de bebedor: " + nombre + ": " + tuplasInsertadas + " tuplas insertadas");

			return new Cliente (numDoc, tipoDocumento, correo, nombre);
		}
		catch (Exception e)
		{
			//        	e.printStackTrace();
	...
	public Hotel adicionarHotel(long pId, String pNombre, String pPais, int pCapacidad) 
	{
		PersistenceManager pm = pmf.getPersistenceManager();
		Transaction tx=pm.currentTransaction();
		try
		{
			tx.begin();
			long tuplasInsertadas = sqlHotel.adicionarHotel(pmf.getPersistenceManager(), pId, pNombre, pPais, pCapacidad);
			System.out.println(tuplasInsertadas);
			System.out.println("papi");
			tx.commit();

			log.trace ("Inserción de bebedor: " + pNombre + ": " + tuplasInsertadas + " tuplas insertadas");

			return new Hotel(pId, pNombre, pPais, pCapacidad);
		}
		catch (Exception e)
		{
			//        	e.printStackTrace();
	...
	public Habitacion adicionarHabitacion(long id, long idHotel, String tipo, char reservada, int capacidad, double costo, long idCliente, long idServicio) {
		PersistenceManager pm = pmf.getPersistenceManager();
		Transaction tx=pm.currentTransaction();
		try
		{
			tx.begin();
			long tuplasInsertadas = sqlHabitacion.adicionarHabitacion(pm, id, idHotel, tipo, idCliente, reservada, idServicio, costo, capacidad);
			tx.commit();

			log.trace ("Inserción de bebedor: " + tipo + ": " + tuplasInsertadas + " tuplas insertadas");

			return new Habitacion(id, idHotel, tipo, reservada, capacidad, costo, idCliente, idServicio);
		}
		catch (Exception e)
		{
			//        	e.printStackTrace();
	...
	public TipoHabitacion adicionarTipoHabitacion(int pId, String pDesc)
	{
		PersistenceManager pm = pmf.getPersistenceManager();
        Transaction tx=pm.currentTransaction();
        try
        {
            tx.begin();
            long tuplasInsertadas = sqlTipoHabitacion.adicionarTipoHabitacio(pm, pId, pDesc);
            tx.commit();
            
            log.trace ("Inserción de tipo de habitación: " + pDesc + ": " + tuplasInsertadas + " tuplas insertadas");
            
            return new TipoHabitacion (pId, pDesc);
        }
        catch (Exception e)
        {
//        	e.printStackTrace();
