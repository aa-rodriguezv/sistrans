...
public class Parranderos 
{
	...
	public Hotel adicionarHotel(long id, String nombre, String pais, int capacidad) {
		log.info("Añadiendo hotel");
		Hotel hotel = pp.adicionarHotel(id, nombre, pais, capacidad);
		log.info ("Adicionando hotel: " + hotel);
	    return hotel;
	}
	
	public TipoHabitacion adicionarTipoHabitacion(int pId, String pDesc) {
		log.info("Añadiendo tipo de habitación");
		TipoHabitacion tipo = pp.adicionarTipoHabitacion(pId, pDesc);
		log.info("Adicionando tipo de habitación: " + pDesc);
		return tipo;
	}
