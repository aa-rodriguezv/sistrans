...
import uniandes.isis2304.parranderos.negocio.VOTipoHabitacion;
...
public class InterfazParranderosApp extends JFrame implements ActionListener
{
	...
	public void adicionarHotel() {
		try {
			String x = JOptionPane.showInputDialog(this, "id del hotel?", "Adicionar el id del hotel", JOptionPane.QUESTION_MESSAGE);
			String nombre = JOptionPane.showInputDialog(this, "nombre del hotel?", "Adicionar el nombre del hotel", JOptionPane.QUESTION_MESSAGE);
			String pais = JOptionPane.showInputDialog(this, "pais del hotel?", "Adicionar el pais del hotel", JOptionPane.QUESTION_MESSAGE);
			String y = JOptionPane.showInputDialog(this, "capacidad del hotel?", "Adicionar la capacidad del hotel", JOptionPane.QUESTION_MESSAGE);
			if(x != null && nombre != null && pais != null && y != null) {
				long id = Long.parseLong(x);
				int cantidad = Integer.parseInt(y);
				VOHotel h = parranderos.adicionarHotel(id, nombre, pais, cantidad);
				if (h == null)
				{
				...
				String resultado = "En adicionarHotela\n\n";
				resultado += "hotel adicionado exitosamente: " + h;
				resultado += "\n Operación terminada";
				panelDatos.actualizarInterfaz(resultado);
			}
		}
	...
	public void adicionarTipoHabitacion() {
		try {
			String id = JOptionPane.showInputDialog(this, "Identificación del tipo de habitación?", "Adicionar el tipo de habitación", JOptionPane.QUESTION_MESSAGE);
			String pDesc = JOptionPane.showInputDialog(this, "nombre del tipo de habitación?", "Adicionar el tipo de habitación", JOptionPane.QUESTION_MESSAGE);
			if(id != null && pDesc != null) {
				int pId = Integer.parseInt(id);
				VOTipoHabitacion t =  parranderos.adicionarTipoHabitacion(pId, pDesc);
				if(t == null) {
					throw new Exception ("No se pudo crear un tipo de habitacion con nombre: " + pDesc);
				}
				String resultado = "En adicionarHotela\n\n";
				resultado += "hotel adicionado exitosamente: " + t;
				resultado += "\n Operación terminada";
				panelDatos.actualizarInterfaz(resultado);
			}
		}catch(Exception e) {
